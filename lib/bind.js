Silo.Bind = new function(){
    ...
    this.element = function(element, scope) {
		...
		if (!Silo.View.getFromScope(str, scope)) return;
		var str = i.attr('silo-bind').replace(/\.\w+$/, '');
		var obj = Silo.View.getFromScope(str, scope);
		switch (i.element.nodeName.toLowerCase()) {
			case 'select': case 'input': case 'textarea':
				i.element.addEventListener('change', function(obj, str) {
