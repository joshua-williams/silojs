Silo.Cache = new function(){
    ...
    this.each = function(element, collection){
        if(is_element(element)){
            if(this.each.items.indexOf(element) === -1) return false;
            return this.each.items[this.each.items.indexOf(element)];
        }else{
            if(!is_array(collection)) return false;
            this.each.items.push({element:element, collection:collection});
            return this.each.items.length -1;
        }
