...
Silo.View.renderElement = function(element, reload){
    ...
    var scope = Silo.scope(element);
    var directiveTags = $dom(element).find('silo\\:controller,silo\\:include,silo\\:view, silo\\:if, [silo-each]:not([silo-each-root]), [silo-bind]');
            ...
            switch(dt.element.nodeName.toLowerCase()){
                ...
                default:
                    if(dt.attr('silo-each')){
                        ...
                        return this.renderElement(element);
                    }else if(dt.attr('silo-bind')){
                    	Silo.Bind.element(dt.element, Silo.scope(dt.element));
                    	return this.renderElement(element)
                    }
            }
        }
    ...
    (function(element){
        ...
        for(var a= 0, e; e=elements[a]; a++){
            ...
            if((callbackName = e.attr('silo-click'))){
                ...
                var event = 'dblclick';
            }else if((callbackName = e.attr('silo-change'))){
            	var event = 'change';
            }else{ continue;}
